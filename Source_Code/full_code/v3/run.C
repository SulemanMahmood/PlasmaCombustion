...
Main::Main(CkArgMsg* m){
  ...
  // call read_file();
	...
	//CkArrayOptions opts(CkArrayIndex4D(0,0,0,0),
										//	CkArrayIndex4D(dimW,dimX+1,dimY,dimZ),
											//CkArrayIndex4D(1,1,1,1));
	//interfaceProxy = CProxy_Interface::ckNew(opts);
	interfaceProxy_x = CProxy_Flux::ckNew(dimX+1,dimY,dimZ);
	interfaceProxy_y = CProxy_Flux::ckNew(dimX,dimY+1,dimZ);
	interfaceProxy_z = CProxy_Flux::ckNew(dimX,dimY,dimZ+1);
	interfaceProxy_x.solve_i();
	interfaceProxy_y.solve_i();
	interfaceProxy_z.solve_i();
}
